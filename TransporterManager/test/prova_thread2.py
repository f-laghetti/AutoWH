...
class transporterController (Thread):
   def __init__(self, tr, goal, goal2, sectors, order_type): #TODO passare crosses e sector_queue
      ...
      self.first_goal = goal #lista dei first_goal
      ...
      self.second_goal = goal2 #lista dei second_goal
   ...
   def transitionCallback(self, addr):
      ...
      if self.first_goal_flag != len(self.first_goal) and nextSec.id == self.first_goal[self.first_goal_flag]:
      ...
      if self.first_goal_flag == len(self.first_goal) and self.second_goal_flag != len(self.second_goal) and nextSec.id == self.second_goal[self.second_goal_flag]:
      ...
      if (self.first_goal_flag != len(self.first_goal) and nextNextSec.id == self.first_goal[self.first_goal_flag]) or (self.first_goal_flag == len(self.first_goal) and self.second_goal_flag != len(self.second_goal) and nextNextSec.id == self.second_goal[self.second_goal_flag]): #prossimo sector è un goal
...
if __name__ == "__main__":
   connect_transporter("c0:e2:c0:f8:39:76", '22', True, 3)
   #connect_transporter("c3:76:2f:0b:70:18", '22', False, 3)
   #connect_transporter("c3:10:2a:28:10:45", '15', False, 3)

   thread1 = transporterController(transporters[0], [('13',[0,1,2]),('18',[0,1,2])] , ['15'], sectors, "DELIVER_SHELF")
