...
COLORS_DICT = {
    ...
    'blue': (0, 0, 200),
    'orange': (200, 100, 0),
    'lavender': (200, 100, 200),
    'greenwater': (100, 200, 100),
    'off': (0, 0, 0),
}
...
class Overdrive:
    ...
    def setChangeLane(self, speed, accel, offset):
        self.setLocationChangeCallback(self._locationChangeCallback)
    ...
    def uTurn(self):
        self.setLane(0.0)
        command = struct.pack("<BHH", 0x32, 4, 0)
        self.sendCommand(command)

    def turnRight(self):
        # time.sleep(0.1)
        self.changeLane(9500, 9200, 61)

    def turnLeft(self):
        """self.setLane(lane)
        command = struct.pack("<BHH", 0x32, 1, 0)
        self.sendCommand(command)"""
        # time.sleep(0.1)
        # self.changeLaneLeft(900,900)
        self.changeLane(9500, 9300, -63)

    def light(self, color='yellow'):
        if color == 'front_red':
            command = struct.pack("<BBBBBBBBBBBBBBBBB",
                                  0x33,
                                  1,  # channel count
    ...
    def enableNotify(self):
        ...
        while True:
            ...
            self.ping()
            self._peripheral.waitForNotifications(3)
            if self.getNotificationsReceived() > 0:
                break
            logging.getLogger("anki.overdrive").error("Connecting please wait...")
    ...
    def _locationChangeCallback(self, location, piece, speed, clockwise, offset):
        ...
        if self._locationChangeCallbackFunc is not None:
            self._locationChangeCallbackFunc(self.addr, location, piece, speed, clockwise, offset)

    def startBatteryLevelPing(self):
        threading.Thread(target=self._batteryLevelCallback).start()
    ...
    def _batteryLevelCallback(self):
        # aggiorna il livello della batteria ogni 60 secondi
        while True:
            if self._connected:
        ...
        if self.handle == handle:
            ...
            if commandId == 0x27:
                ...
                if clockwiseVal == 0x47:
                    clockwise = True
                threading.Thread(target=self.overdrive._locationChangeCallback,
                                 args=(location, piece, speed, clockwise, offset)).start()
            elif commandId == 0x29:
                # Transition notification
                piece, piecePrev, offset, direction = struct.unpack_from("<BBfB", data, 2)
                self.overdrive.offset = offset
                ...
                # Intersection update
                _, self.overdrive.offset, self.overdrive.intersection_code, self.overdrive.is_exiting = struct.unpack_from(
                    "<bfBB", data, 2)
            elif commandId == 0x1b:
                batteryLevel = struct.unpack_from("<H", data, 2)
                self.overdrive.batteryLevel = int((batteryLevel[0] / 4200) * 100)
