...
class deliverShelfExecutor (Thread): #thread che si occupa di gestire un ordine di deliver_shelf
    ...
    def __init__(self, handler, command):
        ...
        self.total_remaining_shelves = command[4] #la lista delle shelves, che verrà aggiornata ogni volta che un transpoter consegna della merce in una shelf

        self.tmp_remaining_shelves = command[4][:] #crea una copia della lista delle shelves che verrà usata per i calcoli nell'assegnamento delle scaffalature ai transactor#lista delle shelves rimanenti
    ...
    def send_to_transactor(self):
        ...
        while self.retrieve_transanctor:
            ...
            if len(unused_transporters) != 0: #se è stato trovato almeno un transporter unused
                ...
                assigned_transporters = self.assignShelves(chosen_transporters, self.tmp_remaining_shelves)
                ...
                for n in range(len(assigned_transporters)): #cicla tra i transporter scelti
                    ...
                    tmp_deliver_shelf_controller = transporterControllerDeliverShelf(tmp_assigned,tmp_diz_shelf,tmp_diz_transactor,self.handler.sectors,"DELIVER_SHELF",self.handler.crosses_queue,self.handler.sectors_queue, self)
...
class transporterControllerDeliverShelf (Thread):
   def __init__(self, tr, goal, goal2, sectors, order_type, crosses, sectors_queue, handler):
      ...
      self.transactor_goal = goal2 #dizionario dei first_goal, ad ogni key corrispondono i livelli da riempire
      self.transactor_goal_flag = 0 #indica il prossimo first_goal da completare; quando sarà uguale alla lunghezza di first_goal vorrà dire che tutti i first_goal sono stati raggiunti
      self.shelves_goal = goal #dizionario dei second_goal, ad ogni key corrisponde il numero di pezzi da rilasciare nel transactor
      self.shelves_goal_flag = 0 #indica il prossimo second_goal da completare
      self.rest = False
   ...
   def transitionCallback(self, addr):
      ...
      if self.transactor_goal_flag != len(self.transactor_goal) and nextSec.id == list(self.transactor_goal)[self.transactor_goal_flag]:
         print("ARRIVATO NEL " + str(self.transactor_goal_flag + 1) + "o TRANSACTOR", nextSec.id)
         ...
         self.handler.num_wares -= self.transactor_goal[list(self.transactor_goal)[self.transactor_goal_flag]][0] #scala le merci prese dal transactor dal conto totale
         print("PRELEVATO DA", list(self.transactor_goal)[self.transactor_goal_flag], "UN AMMONTARE DI", self.transactor_goal[list(self.transactor_goal)[self.transactor_goal_flag]], "PEZZI")
         self.transactor_goal_flag += 1 #aumenta il flag che indica a quale first_goal i è arrivati
      ...
      if self.transactor_goal_flag == len(self.transactor_goal) and self.shelves_goal_flag != len(self.shelves_goal) and nextSec.id == list(self.shelves_goal)[self.shelves_goal_flag]:
         ...
         sleep(3.0)  # simula l'esecuzione del lavoro in 3 secondi
         #TODO scalare le shelf riempite
         for n in range(len(self.handler.total_remaining_shelves)):
             if self.handler.total_remaining_shelves[n].coord == list(self.shelves_goal)[self.shelves_goal_flag]: #controlla che sia la scaffalatura in cui ha depositato la merce
                 for m in range(len(self.shelves_goal[list(self.shelves_goal)[self.shelves_goal_flag]])):
                     print(self.handler.total_remaining_shelves[n].levels)
                     print(self.shelves_goal[list(self.shelves_goal)[self.shelves_goal_flag]][m])
                     self.handler.total_remaining_shelves[n].levels.remove((self.shelves_goal[list(self.shelves_goal)[self.shelves_goal_flag]][m])) #rimuove tutti i livelli di quella shelf dove ha depositato della merce
         print("RIEMPITO LA SCAFFALATURA", list(self.shelves_goal)[self.shelves_goal_flag], "NEI LIVELLI", self.shelves_goal[list(self.shelves_goal)[self.shelves_goal_flag]])
      ...
      if (self.transactor_goal_flag != len(self.transactor_goal) and nextNextSec.id == list(self.transactor_goal)[self.transactor_goal_flag]) or (self.transactor_goal_flag == len(self.transactor_goal) and self.shelves_goal_flag != len(self.shelves_goal) and nextNextSec.id == list(self.shelves_goal)[self.shelves_goal_flag]): #prossimo sector è un goal
...
class transporterControllerRetrieveShelf (Thread):
   def __init__(self, tr, goal, goal2, sectors, order_type, crosses, sectors_queue):
      Thread.__init__(self)
      self.kill_thread = False
      self.tr = tr
      #si dividono i goal in first e second perchè ogni ordine avrà due tipi diferenti di goal: transactor e shelf
