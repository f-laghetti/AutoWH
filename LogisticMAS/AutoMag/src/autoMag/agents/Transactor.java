...
public class Transactor extends Agent{
	...
	public void setup() {
		...
		if (arg != null && arg.length == 3) {
			...
			addBehaviour(new TransactionEndListener());
		}
		else{
	...
	private class TransactionEndListener extends CyclicBehaviour{
		
		ACLMessage transactionEndMsg;
		MessageTemplate mtTransactionEndMessageTemplate = MessageTemplate.and(MessageTemplate.MatchPerformative(ACLMessage.CFP), MessageTemplate.MatchConversationId("transactor-free"));
		
		public void action() {
			transactionEndMsg = myAgent.receive(mtTransactionEndMessageTemplate);
			if(transactionEndMsg != null) {				
				info.setFree();
				ACLMessage reply = transactionEndMsg.createReply();
				System.out.println("Transactor set free");
				reply.setPerformative(ACLMessage.AGREE);
				reply.setContent("Transactor set free");
				myAgent.send(reply);
				transactionEndMsg = null;
			}
			else {
				block();
			}
			...
			if(requestSent == false) {
				...
				try {
					...
					if(warehouseAID != null) {
						...
						request.setContent(info.getID()+":"+info.getCoord().getSector());
